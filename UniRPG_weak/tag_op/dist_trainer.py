...
def main():
    ...
    parser.add_argument('--port', type=int, default=54343)
    ...
    dist.init_process_group('nccl', init_method=init_method, rank=get_local_rank(), world_size=2)
    if get_local_rank() != 0:
        dist.barrier() 
...
#     callbacks.append(FitlogCallback(dev_data_bundle))
    ...
    trainer = DistTrainer(train_data=train_data_bundle, model=model, optimizer=optimizer,
                      loss=Seq2SeqLoss(), batch_size_per_gpu=batch_size_per_gpu, sampler=sampler, 
